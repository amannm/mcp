...
MCP implementations **MUST** validate TLS certificates when using HTTPS transports.

- Custom trust stores **SHOULD** be supported for server certificate validation
- Clients **SHOULD** support client certificates for mutual TLS authentication
- Certificate validation **MAY** be disabled for development, but **MUST** default to strict mode
- Implementations **MAY** pin server certificates by SHA-256 fingerprint
- Server Name Indication (SNI) **MUST** be enabled for HTTPS connections
